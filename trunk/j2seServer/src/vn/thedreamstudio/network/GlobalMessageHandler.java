...
import vn.thedreamstudio.game.network.GameMessageHandler;
...
//                }
    ...
    private void doLogout(Player p) throws IOException {
        FileManager.updateUserInfo(p);
...
//        }
    ...
    public void onDisconnected(Player user) {
        try {
            ...
            Logger.getLogger(GlobalMessageHandler.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void doLoginFromFile(Player user, Message message) throws IOException {
        ...
        if (data != null) {
			...
			user.winNumber = data.winNumber;
            user.loseNumber = data.loseNumber;
        }
